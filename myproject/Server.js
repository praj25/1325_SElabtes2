...
app.get('/testAngularJS',function(req,res){
	  res.sendfile('View/testAngularJS.html');
});
...
**************************************************************************************************************************/
/*app.get('/loadFollowersStories',function(req,res){
...
//select count(u.trip_id) as count, u.* from user_stories u inner join like_info l on  u.trip_id=l.trip_id  where u.user_id in (select follower_id from follower where user_id=31) group by u.trip_id
/*app.get('/loadFollowersStories',function(req,res){
	connection.query("SELECT count(u.trip_id) as totalLikes, u.* from user_stories u inner join like_info l on  u.trip_id=l.trip_id  where u.user_id in (SELECT follower_id from follower where user_id='" + sessions.user_id + "') group by u.trip_id",function(err,rows){
		if(err){
			console.log("ERROR in database: "+err);
		}
		else{
...
app.post('/insertUserStory',function(req,res){
    ...
    connection.query("INSERT INTO `user_stories`(`user_id`,`tripName`,`destination`,`source`,`description`) VALUES ('" + user_id + "','" + tripName + "','" + source + "','" + destination + "','" + description +"')",function(err,result){   
    	if(!!err)
    		data["item"] = "Error Adding data";
        else{
            data["error"] = 0;
